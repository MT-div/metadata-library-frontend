...
import { LayoutTemplate, Plus, Save, ArrowUp, ArrowDown, Trash2, Settings2 } from 'lucide-react';
import type { ResourceTemplateResponse, TemplatePropertyRequest } from '../../types/metadata';
...
export const ManageTemplatesPage = () => {
  ...
  const [editableProperties, setEditableProperties] = useState<EditableProperty[]>([]);
  ...
  const selectTemplate = (templateId: number | null, templatesList: ResourceTemplateResponse[] = templates) => {
    ...
    if (template) {
      const mappedProps: EditableProperty[] = template.properties.map(p => ({
  ...
  const handleRemoveProperty = (indexToRemove: number) => {
    const updated = editableProperties.filter((_, idx) => idx !== indexToRemove);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/templates/${selectedTemplateId}/properties`, {
        method: 'PUT',
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 border-b pb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
            <Settings2 className="text-primary" size={32} /> إدارة وتعديل القوالب
          </h1>
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-white rounded-xl shadow-sm border p-5">
            <h2 className="font-bold text-lg text-gray-800 flex items-center gap-2 mb-4">
              <LayoutTemplate className="text-primary" size={20} /> القوالب المتاحة
            </h2>
          ...
          {selectedTemplate ? (
            <div className="bg-white rounded-xl shadow-sm border overflow-hidden flex flex-col h-full">
              ...
              <div className="bg-gray-50 border-b p-5 flex justify-between items-center">
                <div>
                  <h2 className="font-bold text-xl text-gray-800">محرر الحقول: {selectedTemplate.label}</h2>
                ...
                >
                  <Save size={20} /> {isSaving ? 'جاري الحفظ...' : 'حفظ التعديلات'}
                </button>
              </div>

              {/* أداة إضافة خاصية جديدة */}
              <div className="p-5 border-b bg-white">
                <label className="block text-sm font-semibold mb-2 text-gray-700">إضافة خاصية لهذا القالب:</label>
                <div className="flex gap-2">
                  <select 
                    className="flex-grow border rounded-lg p-2.5 outline-none bg-white focus:border-primary"
                    value={propToAdd}
                    onChange={(e) => setPropToAdd(Number(e.target.value))}
                  >
                    <option value={0}>-- اختر خاصية من القائمة --</option>
                    {allProperties.map(p => (
                      <option key={p.id} value={p.id}>{p.label} ({p.vocabularyPrefix}:{p.localName})</option>
                    ))}
                  </select>
                  <button onClick={handleAddProperty} className="bg-green-600 text-white px-5 py-2.5 rounded-lg font-medium hover:bg-green-700 flex items-center gap-2 transition">
                    <Plus size={18} /> إضافة
                  </button>
                </div>
              </div>
              ...
              <div className="p-5 flex-grow">
                {editableProperties.length === 0 ? (
                  <p className="text-center text-gray-500 py-10">القالب فارغ، يرجى إضافة خصائص.</p>
                ) : (
                    ...
                    <table className="w-full text-right text-sm">
                      ...
                      <tbody>
                        {editableProperties.map((prop, idx) => (
                          <tr key={prop.propertyId} className="border-t hover:bg-gray-50 transition bg-white">
                            <td className="p-3 text-center font-bold text-gray-400">{prop.displayOrder}</td>
                            <td className="p-3 font-semibold text-primary text-base">{prop.propertyLabel}</td>
                            ...
                            <td className="p-3">
                              <div className="flex justify-center gap-1">
                                ...
                                <button 
                                  ...
                                  disabled={idx === editableProperties.length - 1}
                                  className="p-1.5 bg-gray-100 rounded hover:bg-gray-200 disabled:opacity-30 disabled:cursor-not-allowed transition"
                                  title="تحريك للأسفل"
                                >
